...
-----------------------------------------------------------------------
```{r}
boros = inspections %>% distinct(boro) %>% pull()
...
inspections$inspection_date <- as.POSIXct(inspections$inspection_date, format = "%Y-%m-%d")
 ...
 renderPlotly( {ggplotly(
  ...
  inspections %>%
    na.omit() %>%
  filter(boro == input$boro_choice, inspection_date %in% input$date_range[1]:input$date_range[2]) %>%
    ggplot(aes(x = as.POSIXct(inspection_date), y = score)) +
      geom_hex() +
